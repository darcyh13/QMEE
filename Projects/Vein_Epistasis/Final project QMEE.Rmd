...
We lastly fit a model using a multivariate approach to predict the vein length for each vein. This allows for us to model differences in vein length with muliple correlated response factors ( veins 2 -5). Multivariate approaches have been shown to be better than fitting a single model for each response variable and we could address not only how big an effect is but how similar is it in each vein. However, this wasn't a powerful advantage for this project as we were more concerned if there was an overall effect of background dependence and not how this effect impacted each vein individually and comparisons between the veins.A downside to this approach is the outputs can be hard to interpret.
